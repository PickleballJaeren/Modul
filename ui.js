...
import { db, SAM, updateDoc, doc } from './firebase.js';
...
export function naviger(skjerm, retning = null) {
  ...
  // Lazy-lasting av skjerminnhold importeres i app.js for å unngå sirkulære avhengigheter
  document.dispatchEvent(new CustomEvent('navigert', { detail: { skjerm } }));
}
...
export function startFailSafe() {
  clearTimeout(failSafeTimer);
  failSafeTimer = setTimeout(async () => {
    console.warn('[Lås] Fail-safe utløst — løser lås automatisk');
    if (app.treningId && db) {
      try { await updateDoc(doc(db, SAM.TRENINGER, app.treningId), { laast: false }); }
...
window.addEventListener('beforeunload', e => {
  if (app.treningId) { e.preventDefault(); e.returnValue = ''; }
});
